...
void render(const int img_w, const int img_h, std::shared_ptr<Camera> camera, lux::SField& g, std::vector<std::shared_ptr<lux::Light>>& lights)
{
	...
	lux::SField to1 = std::make_shared<lux::SFTorus>(0.5, 0.05, lux::Vector(), lux::Vector(-1, 1, 1));
	//to1 = std::make_shared<lux::SFTranslate>(to1, lux::Vector(-0.8, -0.3, 0.0));
	lux::SField to2 = std::make_shared<lux::SFTorus>(0.5, 0.05, lux::Vector(), lux::Vector(1, 1, 1));
	...
	lux::SField pl2 = std::make_shared<lux::SFPlane>(lux::Vector(), lux::Vector(1.0, 0.0, 0.0));
	lux::SField pl3 = std::make_shared<lux::SFPlane>(lux::Vector(), lux::Vector(-1.0, 0.0, 0.0));
	lux::SField bx = std::make_shared<lux::SFBox>(1);
	lux::SField el = std::make_shared<lux::SFEllipse>(0.3, 0.5, lux::Vector(0.0, 0.0, 0.0), lux::Vector(1.0, 0.0, 0.0));
	...
	key->setColor(lux::Color(235.0 / 255.0, 16.0 / 255.0, 16.0 / 255.0, 1.0));
	std::shared_ptr<lux::Light> fill = std::make_shared<lux::Light>(lux::Vector(0.0, -1.0, 0.5),
		lux::Vector(-o, -o, -o), 3 * l / 5, 3 * l / 5, 3 * l / 5, 2 * o / (3 * l / 5));
	fill->setColor(lux::Color(138.0 / 255.0, 7.0 / 255.0, 7.0 / 255.0, 1.0));
	lights = { key, fill };
...
lux::Color marchRays(lux::Vector pos, lux::Vector dir, const lux::SField& g, const std::vector<std::shared_ptr<lux::Light>>& lights)
{
	...
	lux::Color bloodRed(235.0 / 255.0, 16.0 / 255.0, 16.0 / 255.0, 1.0);

	float sNear = 0.2, sFar = 6.0;
	float T = 1;
	float delta_s = 0.01;
	float delta_T;
	float kappa = 80; //0.1 for wisp // 800
	float s = sNear;
